...
// Kiêm tra xem danh mục nào
var danhMuc = localStorage.getItem("danh muc");
var tenListDanhMuc = [productsTreEm, productsNguoiLon, productsFE, productsMicro];
...
var products = tenListDanhMuc[0];
function checkDanhMuc() {
    for (let i = 0; i < tenDanhMuc.length; i++) {
        if (danhMuc === tenDanhMuc[i]) {
            products = tenListDanhMuc[i];
            renderProduct();
            return;
        }
    }
...
// Hàm hiển thị số trang (3 2 1 trang)
